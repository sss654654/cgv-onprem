...
```
 요청: { screeningId, seatNos, requestId, idempotencyKey }
   ...
   │
   ├─ ③ 게이트 ─────────────────────────────────────────────
   │    입장 인증 있나?  없으면 403 · Redis 가 응답 못 하면 503(result=redis_error)
   │
   ├─ ④ 좌석 실재·판매완료 확인 ────────────────────────────
   │    없는 좌석 → 400 / 이미 팔린 좌석 → 409
   │
   ├─ ⑤ 락 재확인·갱신 ─────────────────────────────────────
   │    renewMine(내 좌석 TTL 연장)   실패 → 409 · Redis 가 응답 못 하면 503(result=redis_error)
   │    ③ · ⑤ 는 결제 전이라 503 이어도 돈이 움직이지 않는다. Redis 명령 타임아웃은 2초(application.yml)
   │
   ├─ ⑥ PG 승인 (동기, 트랜잭션 밖) ────────────────────────
...
**멱등이 게이트보다 앞이다.** 확정에 성공하면 인증이 소진되므로, 게이트가 앞에 있으면 아래 경로가 막힌다.

```
상황            결제는 됐는데 응답이 유실돼 클라이언트가 같은 멱등키로 재시도
게이트가 앞이면   403.  돈은 빠졌는데 예매번호를 못 받고,
                지표에도 replay 가 아니라 forbidden 으로 찍힌다
```

멱등키 조회를 앞에 둬도 안전한 이유 — 상태를 바꾸지 않고, 키는 요청자만 아는 값이며, 회차·좌석이 다르면 뒤에서 409로 거절한다.
...
**점유 좌석은 회차별 인덱스 Set으로 센다.** `seatlocks:{screeningId}`에 점유 중인 좌석번호를 좌석락과 같은 Lua 안에서 함께 기록한다.

인덱스가 없으면 "이 회차에 몇 석 잠겼나"를 `SCAN`으로 알아내야 한다.

```
SCAN 의 MATCH   서버측 필터일 뿐, 커서는 DB 전체 키를 훑는다
비용            회차 하나 조회에 (전체 키수 / COUNT) 회 왕복
그리고          이 Redis 는 queue 가 대기열 폴링을 처리하는 바로 그 인스턴스다
```

같은 이유로 **회차 목록은 판매수도 점유수도 회차별로 묻지 않는다.**

```
판매수   group by 한 번
점유수   회차 키 전부를 KEYS 로 받는 Lua 한 번
```

회차 목록은 입장 직후 첫 화면이라 정원 전체가 주기적으로 다시 부른다. 회차가 20개(지점5 × 관4)라 회차별로 물으면 요청 하나가 Redis를 20번 왕복한다.
...
**그 위에 짧은 캐시와 single-flight가 얹혀 있다.** 왕복을 줄여도, 대기 중인 전원이 같은 주기로 이 경로를 부르면 캐시가 만료되는 순간 전원이 동시에 계산을 시작한다.

```
고리   계산이 DB 커넥션을 쓴다 → 풀 마비 → 계산이 느려진다 → 대기가 더 쌓인다
실측   공개 경로 부하 판에서 5xx 초당 467건.  actuator 조차 응답하지 못했다
조치   만료를 본 요청 중 하나만 계산한다 (AtomicBoolean.compareAndSet)
       나머지는 직전 값을 즉시 받는다
풀     그대로 뒀다 — 수요 쪽 결함이라 늘렸으면 같은 폭주가 더 큰 규모로 났다
```
...
- **재시도는 2초 간격으로 5회다**(`FixedBackOff(2000, 5)`, 첫 시도를 합쳐 6번). 재시도는 리스너 스레드를 붙잡으므로 그 컨슈머가 맡은 파티션 전체가 그동안 멈춘다. 멈추는 최대 시간은 시도마다 Redis 명령 타임아웃(2초)까지 기다리는 경우다 — 시도 6번 × 2초 + 간격 5번 × 2초 = 22초. 창을 짧게 잡아 Redis 파드 재기동(수 초)은 흡수하되 파티션 정지를 그 안으로 묶는다.
...
표의 값   코드 기본값
배포값    180 < 300 < 600
근거      부하 판에서 한 사람의 여정을 쟀다 —
          회차 조회 10-30초 + 좌석 선택 30-120초 + 결제 60-180초
```

순서가 깨지면 자원이 멀쩡해도 좌석 선택이 403이 되어 처리량만 무너진다.
