...
KBO 야구의 온라인 예매가 폭주해 예매 서버가 수요를 받아내지 못한 실제 사건을 모델로 한다.

```
보도·후기   예매 버튼이 활성화되지도 못한 채 대기 인원 16만 명
            좌석은 몇 분 만에 전량 매진
공식 발표   없다.  정황은 후기·보도로만 확인된다
이후        CGV 가 극장에서 야구 생중계 예매 서비스를 열었다
```

원인은 "대기열·용량 산정 부실 → 수요가 백엔드를 직격"으로 본다. 공식 발표가 없어 이건 추정이다.
...
- **세 축이 서로 이어져 있다**
  ...
  ```
  지표 → 트레이스    p99 그래프의 exemplar 를 누르면 그 요청의 트레이스가 열린다
  트레이스 → 로그    그 요청이 남긴 로그로 건너간다
  트레이스 안        HTTP · Kafka 뿐 아니라 SQL 구간과 Redis 명령이 span 으로 들어 있다
                    예매 확정 한 건이 21개 span 으로 쪼개진다
  서비스 경계        입장 승격 → 인증 발급, 예매 확정 → 자리 반환.
                    Kafka 를 넘어도 한 트레이스로 이어진다
  ```
- **트레이스 표본** — booking·queue 모두 전 요청(`OTEL_TRACES_SAMPLER_ARG=1.0`)이고 프로브만 만들지 않는다.

  queue 폴링이 요청의 97%라 한동안 1%로 두었다. 표본을 올려도 exemplar 개수가 늘지 않았는데, 스크레이프(15초)마다 히스토그램 버킷당 하나가 한도라 표본이 아니라 그 한도가 개수를 정하기 때문이다. 느린 요청을 훑는 수단을 Tempo 검색(TraceQL)으로 옮기고 표본을 전부로 올렸다.
- **부하로 스펙을 정했다** — 판을 두 번에 나눠 돌렸다.

  ```
  클러스터 안        21회
  인터넷 공개 후      13회 (같은 경로로)
  나온 것           정원 · 타임아웃 · CPU/메모리 limit · 커넥션 풀
  계수 10개          예: booking CPU = 0.007 + 0.0258 × 확정/초
  ```
