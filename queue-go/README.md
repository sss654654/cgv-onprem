...
```
되돌리는 방식의 전제   발행 실패 = 브로커에 안 들어감
실제                 ctx 취소·타임아웃은 브로커 append 이후에도 에러로 돌아온다
그래서 되돌리면        booking 에는 입장이 들어갔는데 queue 만 자리를 뺏는 상태가 된다
```

저널에 남겨 재발행하는 쪽은 그 경로를 만들지 않는다.
...
원자성이 필요한 연산(정원 확인·승격·만료·이탈)은 Lua로 묶는다. **읽기인 position도 Lua다.**

```
따로 왕복하면      ZSCORE 와 ZRANK 사이에 승격이 끼어든다
결과              waiting 에서 빠진 사람을 "둘 다 없음"으로 읽고,
                  방금 입장한 사용자에게 EXPIRED 를 돌려준다
판정 순서만으로는   그 창이 안 닫힌다
```

`redis/` 패키지가 go-redis 의존을 가둬, 핸들러·프로세서는 래퍼 타입만 참조한다.
...
- **실황 피드가 List가 아니라 ZSet인 이유는 읽는 쪽 조건에 있다.**

  ```
  ZSet   score 를 이벤트 id 로 두면 "id 가 after 보다 큰 것"을 Redis 가 잘라서 보낸다
         새 이벤트가 없으면 0건이 온다
  List   그 조건을 서버에서 못 건다.  보관분 전체(100건, 약 6KB)를 받아 앱에서 잘라야 한다
  ```
  피드는 화면마다 3초로 폴링하는 경로라, List면 그 전송이 대부분 낭비가 된다.
...
- **파티션 키 = `requestId`** — 같은 사용자의 입장·회수가 같은 파티션에서 순서대로 처리된다.

  ```
  키가 없으면   두 이벤트가 다른 파티션에 흩어진다
               → "회수 → 입장" 순으로 처리될 수 있고, 이미 지운 인증이 되살아난다
  사용자 간     순서 의존이 없다.  requestId 단위 분산이 파티션을 고르게 쓰면서
               필요한 순서만 지킨다
  ```
...
- **발행 ack**: `RequiredAcks=RequireAll` + 재시도 3회. 그 파티션의 ISR 전부가 기록해야 성공이라, 리더가 복사 전에 죽어도 성공으로 답한 입장이 남는다(리더만 받는 `RequireOne` 이면 이 경우 조용히 사라지고 저널에도 안 남는다). 살아 있는 사본이 `min.insync.replicas`(2)보다 적거나 재시도가 소진되면 에러를 돌려주고, 호출자는 저널을 남긴다. 대가는 발행 지연이다 — 팔로워 복제까지 기다린다.
...
승격 배치와 주기는 **곱이 상한을 정한다** — `25 / 0.5초`와 `100 / 2초`는 둘 다 초당 50명이다.
`25 / 0.5초`를 고른 이유는 상한이 아니라 **한 번에 나가는 뭉텅이 크기**다.

```
오픈 순간     정원이 통째로 비어 있어 배치가 그대로 한꺼번에 발행된다
뭉텅이 100    그 순간 Kafka 로 나가는 덩어리가 네 배 → 소비 쪽 지연이 그만큼 길어진다
실측          공개 경로 판에서 인증 지연 p99 4.971초 → 0.988초
```
